# Test of Chinese as a Foreign Language Listening Band C (Vol.1)
# Message: Terribly sorry for the delayed questions, some questions are equipped with 2 audios, so it's safe to say the delay is for the second audio to finish then you can answer...

import sys
import time
import subprocess
import threading

from pathlib import Path
from termcolor import colored
from PIL import Image
from mutagen.mp3 import MP3

# Path setup (SAFE)

BASE_DIR = Path(__file__).parent
AUDIO_DIR = BASE_DIR / "audio2"
IMAGE_DIR = BASE_DIR / "images"

# Question Bank 

def get_audio_duration(audio_files, audio_dir):
    total = 0.0
    for file in audio_files:
        path = audio_dir / file
        audio = MP3(path)
        total += audio.info.length
    return int(total) + 1  # buffer for safety



questions = [
    {
    "id": 1,
    "audio": ['1-01-0.mp3', '1-01-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "盡快搬家",
        "B": "不付房租",
        "C": "繼續住下去",
        "D": "找人一起租",
    },
    "answer": "C"
},

{
    "id": 2,
    "audio": ['1-02.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "他不要押金了",
        "B": "他決定再住一天就搬家",
        "C": "他要押金，但不打算搬家",
        "D": "他拿到押金以後才要搬家",
    },
    "answer": "D"
},

{
    "id": 3,
    "audio": ['1-03-0.mp3', '1-03-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "能克服天生的障礙",
        "B": "專門為殘障兒童作曲",
        "C": "文章寫得比作家還好",
        "D": "將父親的作品寫成曲子",
    },
    "answer": "A"
},

{
    "id": 4,
    "audio": ['1-04.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "會讓人想睡覺",
        "B": "能幫助人們創作音樂",
        "C": "會讓人們忘了要睡覺",
        "D": "會介紹他兒子的音樂",
    },
    "answer": "A"
},

{
    "id": 5,
    "audio": ['1-05-0.mp3', '1-05-1.mp3'],
    "image": None,
    "question": "這項服務有什麼優點？",
    "choices": {
        "A": "可降低醫療費用",
        "B": "可維護病人隱私",
        "C": "可指定醫生看診",
        "D": "看病可事先預約",
    },
    "answer": "B"
},

{
    "id": 6,
    "audio": ['1-06.mp3'],
    "image": None,
    "question": "這項服務 改善了哪種現象 ？",
    "choices": {
        "A": "病人掛不到號的困境",
        "B": "候診間過度擁擠的 狀況",
        "C": "病人遍尋不著診間的情況",
        "D": "舒緩了 病人候診時的焦慮",
    },
    "answer": "B"
},

{
    "id": 7,
    "audio": ['1-07.mp3'],
    "image": None,
    "question": "關於這項服務，哪一個是對的？ 現在請聽對話。",
    "choices": {
        "A": "所有門診都會提供",
        "B": "使用對象以老人為主",
        "C": "器材用完即丟，不怕傳染",
        "D": "考慮到視障、聽障人士的需求",
    },
    "answer": "D"
},

{
    "id": 8,
    "audio": ['1-08-0.mp3', '1-08-1.mp3'],
    "image": None,
    "question": "關於實施 這項政策， 下面哪一個是對的？",
    "choices": {
        "A": "食品類等產品較不受歡迎",
        "B": "大多數監獄 生產手工藝品",
        "C": "同時考量了 長期服刑的 人",
        "D": "有餐飲背景的受刑人才能參加",
    },
    "answer": "C"
},

{
    "id": 9,
    "audio": ['1-09.mp3'],
    "image": None,
    "question": "這位小姐用統計資料來說明哪一個現象？",
    "choices": {
        "A": "入獄的人數跟過去差不多",
        "B": "有專長的人入獄的比例較低",
        "C": "監獄生產的商品頗受大眾喜 愛",
        "D": "出獄後的再犯率和過去差不多",
    },
    "answer": "D"
},

{
    "id": 10,
    "audio": ['1-10.mp3'],
    "image": None,
    "question": "根據對話，使出獄的人再度犯罪的主要因素是什麼？ 現在請聽對話。",
    "choices": {
        "A": "經濟不景氣",
        "B": "社會接受度不高",
        "C": "缺乏謀生的技能",
        "D": "拿不到社會補助",
    },
    "answer": "B"
},

{
    "id": 11,
    "audio": ['1-11-0.mp3', '1-11-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "因為揭開了沉香成形的祕密",
        "B": "因為找到了沉香不受蟲害的方法",
        "C": "因為發現了提高沉香產量的方法",
        "D": "因為打破了不能人工種植沉香的想法",
    },
    "answer": "C"
},

{
    "id": 12,
    "audio": ['1-12.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "他懷疑研究結果",
        "B": "他讚賞研究發現",
        "C": "他肯定研究的方法",
        "D": "他質疑研究的動機",
    },
    "answer": "B"
},

{
    "id": 13,
    "audio": ['1-13.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "研究時間不只一年",
        "B": "需要進行相關實驗",
        "C": "後續需要張主任的幫忙",
        "D": "她會長期關注之後的發展",
    },
    "answer": "A"
},

{
    "id": 14,
    "audio": ['1-14-0.mp3', '1-14-1.mp3'],
    "image": None,
    "question": "對話中提到，這家網路書店早期所使用的策略是什麼？",
    "choices": {
        "A": "只購買一本書就免運費",
        "B": "將運費包含在書籍 的售價中",
        "C": "運費隨著消費金額提高而增加",
        "D": "消費一定的金額，享有低廉運費",
    },
    "answer": "D"
},

{
    "id": 15,
    "audio": ['1-15.mp3'],
    "image": None,
    "question": "這家網路書店如何改變策略？",
    "choices": {
        "A": "無條件提供免運費服務",
        "B": "加入會員就能享有免運費 服務",
        "C": "提高享有免運費 服務的購物金額",
        "D": "購買指定書籍可享有免運費 服務",
    },
    "answer": "C"
},

{
    "id": 16,
    "audio": ['1-16.mp3'],
    "image": None,
    "question": "這位先生提到的汽水和零食有什麼特色？",
    "choices": {
        "A": "都送贈品",
        "B": "都是免費的",
        "C": "降低廠商 生產成本",
        "D": "讓消費者以為沒有負擔",
    },
    "answer": "D"
},

{
    "id": 17,
    "audio": ['1-17.mp3'],
    "image": None,
    "question": "這位先生認為免費的贈品或服務怎麼樣？ 現在請聽對話。",
    "choices": {
        "A": "不見得划算",
        "B": "能造福消費者",
        "C": "品質一定不好",
        "D": "消費者不一定喜歡",
    },
    "answer": "A"
},

{
    "id": 18,
    "audio": ['1-18-0.mp3', '1-18-1.mp3'],
    "image": None,
    "question": "這個網站能讓人做什麼？",
    "choices": {
        "A": "開個玩笑輕鬆一下",
        "B": "投票選出最出名的人",
        "C": "認識很多有成就的人",
        "D": "把自己介紹給網路上的朋友",
    },
    "answer": "A"
},

{
    "id": 19,
    "audio": ['1-19.mp3'],
    "image": None,
    "question": "關於這個網站所提供的檔案，哪一個是對的？",
    "choices": {
        "A": "具有法律效力",
        "B": "可以免費改變風格",
        "C": "一年內可以隨時申請",
        "D": "只有公益團體可以申請",
    },
    "answer": "C"
},

{
    "id": 20,
    "audio": ['1-20.mp3'],
    "image": None,
    "question": "這個網站怎麼利用賺到的錢？",
    "choices": {
        "A": "平均分給各個公益團體",
        "B": "給最受歡迎的公益團體",
        "C": "成立國際性的公益團體",
        "D": "請名人來幫公益團體募款",
    },
    "answer": "B"
},

{
    "id": 21,
    "audio": ['1-21.mp3'],
    "image": None,
    "question": "這位小姐為什麼希望網站不要公布票選結果？ 現在請聽對話。",
    "choices": {
        "A": "擔心網站侵吞捐款",
        "B": "擔心慈善捐款分 配不均",
        "C": "不願網站藉此打知名度",
        "D": "不願讓別人覺得沒有面子",
    },
    "answer": "B"
},

{
    "id": 22,
    "audio": ['1-22-0.mp3', '1-22-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "繁衍後代和物種進化是否有關",
        "B": "進化的目的 是否和適應環境有關",
        "C": "所有物種 是否只有一套 進化機制",
        "D": "物種進化是否要考量人為的操作",
    },
    "answer": "C"
},

{
    "id": 23,
    "audio": ['1-23.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "深信不疑",
        "B": "質疑問難",
        "C": "嗤之以鼻",
        "D": "同聲讚美",
    },
    "answer": "B"
},

{
    "id": 24,
    "audio": ['1-24.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "學者對「性擇論」 的反彈更大",
        "B": "有學者用數學模型 導出理論的不足",
        "C": "學者們認為補足了「演化論」的缺失",
        "D": "有些學者認為 還能解釋演化的社會性",
    },
    "answer": "A"
},

{
    "id": 25,
    "audio": ['1-25.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "氣溫降低有短暫雨",
        "B": "受到鋒面影響會下雨",
        "C": "是適合出遊踏青的好天氣",
        "D": "早上會下雨，下午出太陽",
    },
    "answer": "A"
},

{
    "id": 26,
    "audio": ['2-26-0.mp3', '2-26-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "寄送速度快",
        "B": "免費寄送郵件",
        "C": "只能寄信給自己",
        "D": "寄送時間很有彈性",
    },
    "answer": "D"
},

{
    "id": 27,
    "audio": ['2-27.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "信被寄丟了",
        "B": "信沒辦法立刻寄到",
        "C": "公司生意太好，影響效率",
        "D": "到了約定的時間，卻沒 人寄信",
    },
    "answer": "D"
},

{
    "id": 28,
    "audio": ['2-28-0.mp3', '2-28-1.mp3'],
    "image": None,
    "question": "朱熹一開始聽到買「東西」時，他的反應是什麼？",
    "choices": {
        "A": "一頭霧水",
        "B": "旁徵博引",
        "C": "胸有成竹",
        "D": "手足無措",
    },
    "answer": "A"
},

{
    "id": 29,
    "audio": ['2-29.mp3'],
    "image": None,
    "question": "朋友從什麼角度來回答朱熹的問題？",
    "choices": {
        "A": "貨品的產地",
        "B": "方位的屬性",
        "C": "籃子的材料",
        "D": "市場的位置",
    },
    "answer": "B"
},

{
    "id": 30,
    "audio": ['2-30.mp3'],
    "image": None,
    "question": "最後，根據朱熹的說法， 「東西」各代表什麼？ 現在請聽故事。",
    "choices": {
        "A": "金和火",
        "B": "木和水",
        "C": "木和金",
        "D": "水和火",
    },
    "answer": "C"
},

{
    "id": 31,
    "audio": ['2-31-0.mp3', '2-31-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "因為無法消滅網路病毒",
        "B": "因為不知何時面臨死亡",
        "C": "因為不了解網路投資的風險",
        "D": "因為想 知道自己的帳號 是否被盜用",
    },
    "answer": "B"
},

{
    "id": 32,
    "audio": ['2-32.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "幫客戶做好電腦備份",
        "B": "協助客戶淘汰很少使用的帳號",
        "C": "讓客戶隨時能和家人分享資料",
        "D": "幫客戶將帳號資料當作遺產處理",
    },
    "answer": "D"
},

{
    "id": 33,
    "audio": ['2-33.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "可以免費使用",
        "B": "可預防電腦中毒",
        "C": "密碼只能給同一個人",
        "D": "可請管理公司代為關閉帳號",
    },
    "answer": "D"
},

{
    "id": 34,
    "audio": ['2-34-0.mp3', '2-34-1.mp3'],
    "image": None,
    "question": "帕梅森提出什麼方法來拯救動植物？",
    "choices": {
        "A": "設立保護區",
        "B": "減輕暖化現象",
        "C": "幫動植物搬家",
        "D": "改善當地的環境",
    },
    "answer": "C"
},

{
    "id": 35,
    "audio": ['2-35.mp3'],
    "image": None,
    "question": "帕梅森為什麼想透過這個方式來拯救動植物 ？",
    "choices": {
        "A": "因為最 有功效",
        "B": "因為十分 經濟",
        "C": "因為能顧及多數物種",
        "D": "因為對生物界的傷害最小",
    },
    "answer": "A"
},

{
    "id": 36,
    "audio": ['2-36.mp3'],
    "image": None,
    "question": "為什麼不少生物學家反對帕梅森提出的方法？",
    "choices": {
        "A": "會破壞生態平衡",
        "B": "會影響氣候變化",
        "C": "人類會大量捕殺 砍伐動植物",
        "D": "拯救這些動植物 無法改善 環境問題",
    },
    "answer": "A"
},

{
    "id": 37,
    "audio": ['2-37.mp3'],
    "image": None,
    "question": "學者們認為帕梅森的方法比較能拯救哪些動植物？ 現在請聽這段話。",
    "choices": {
        "A": "能自行遷徙的動植物",
        "B": "經濟效益低的動植物",
        "C": "沒有絕種危機的動植物",
        "D": "人類認為有價值的動植物",
    },
    "answer": "D"
},

{
    "id": 38,
    "audio": ['2-38-0.mp3', '2-38-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "腦細胞的大小和智商高低無關",
        "B": "腦細胞一出生就已經發展完備",
        "C": "比其他細胞更快速 地進行汰換",
        "D": "存活的 時間不受軀體存活限制",
    },
    "answer": "D"
},

{
    "id": 39,
    "audio": ['2-39.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "腦神經醫療",
        "B": "腦年齡診斷",
        "C": "腦中風治療",
        "D": "腦功能檢測",
    },
    "answer": "A"
},

{
    "id": 40,
    "audio": ['2-40.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "先培育腦細胞，再植入 小家鼠腦中",
        "B": "將小家鼠的腦細胞，植入 年幼大家鼠中",
        "C": "將成年大家鼠 的腦細胞 ，植入小家鼠 中",
        "D": "將腦細胞植入壽命只 剩兩天的小家鼠 中",
    },
    "answer": "B"
},

{
    "id": 41,
    "audio": ['2-41.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "大家鼠的腦神經逐漸萎縮",
        "B": "小家鼠的 壽命比大家鼠 長",
        "C": "大家鼠沒有實驗 後的後遺症",
        "D": "小家鼠的腦細胞 和大家鼠一樣 多",
    },
    "answer": "C"
},

{
    "id": 42,
    "audio": ['2-42-0.mp3', '2-42-1.mp3'],
    "image": None,
    "question": "《南半球跨國農用土地交易》報告，提出了哪方面的問題？",
    "choices": {
        "A": "跨國貿易 衍生的糾紛",
        "B": "跨國農地交易的合理性",
        "C": "投資者購買農地的權利保障",
        "D": "農地交易能否有效提高糧食產量",
    },
    "answer": "B"
},

{
    "id": 43,
    "audio": ['2-43.mp3'],
    "image": None,
    "question": "投資者對於跨國土地投資的看法是什麼？",
    "choices": {
        "A": "宣稱可增加全球糧食產量",
        "B": "可以讓提供農地的國家免於飢荒",
        "C": "有助於提昇 當地居民的生活 水準",
        "D": "能解決 地主國糧食生產不足問題",
    },
    "answer": "A"
},

{
    "id": 44,
    "audio": ['2-44.mp3'],
    "image": None,
    "question": "說話者對於 這些問題 的看法是什麼？",
    "choices": {
        "A": "應該開放貧窮國家的 土地買賣",
        "B": "貧窮國家想脫貧的心態可以理解",
        "C": "土地交易存在太多強欺弱的問題",
        "D": "開墾荒地將對當地環境造成傷害",
    },
    "answer": "C"
},

{
    "id": 45,
    "audio": ['2-45.mp3'],
    "image": None,
    "question": "這篇報導指出 ，一旦發生 交易糾紛， 地主國能怎麼做 ？ 現在請聽報導。",
    "choices": {
        "A": "主動中斷投資 計畫",
        "B": "只能選擇賠錢了事",
        "C": "得先訴諸國際輿論",
        "D": "無法為自己爭取權益",
    },
    "answer": "D"
},

{
    "id": 46,
    "audio": ['2-46-0.mp3', '2-46-1.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "因為矮人暴躁易怒",
        "B": "因為矮人善於 巫術",
        "C": "因為矮人擅長打仗",
        "D": "因為矮人非常團結",
    },
    "answer": "B"
},

{
    "id": 47,
    "audio": ['2-47.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "掠奪矮人的土地",
        "B": "設計陷阱殺 害矮人",
        "C": "讓矮人的婦女懷孕",
        "D": "使用巫術讓矮人懼怕",
    },
    "answer": "B"
},

{
    "id": 48,
    "audio": ['2-48.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "以牙還牙",
        "B": "忍氣吞聲",
        "C": "遠走他鄉",
        "D": "搖尾乞憐",
    },
    "answer": "C"
},

{
    "id": 49,
    "audio": ['2-49.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "慶祝戰鬥的勝利",
        "B": "祈求仇人的諒解",
        "C": "悼念族裡 死去的戰士",
        "D": "將族中婦女 獻給矮人",
    },
    "answer": "B"
},

{
    "id": 50,
    "audio": ['2-50.mp3'],
    "image": None,
    "question": "",
    "choices": {
        "A": "春耕之後",
        "B": "夏種之後",
        "C": "秋收之後",
        "D": "冬藏之後",
    },
    "answer": "C"
},
]

# PATHS
BASE_DIR = Path(__file__).parent
AUDIO_DIR = BASE_DIR / "audio"
IMAGE_DIR = BASE_DIR / "images"

# HELPERS
def normalize_audios(q):
    audios = q.get("audios") or q.get("audio")
    if audios is None:
        raise ValueError(f"Question {q['id']} missing audio")

    if isinstance(audios, str):
        audios = [audios]

    flat = []
    for a in audios:
        if isinstance(a, list):
            flat.extend(a)
        else:
            flat.append(a)

    q["audios"] = flat


def get_audio_duration(audio_files):
    total = 0.0
    for file in audio_files:
        audio = MP3(AUDIO_DIR / file)
        total += audio.info.length
    return int(total) + 1


def play_audio_sequence(audio_files, done_flag):
    for file in audio_files:
        subprocess.call(
            ["afplay", str(AUDIO_DIR / file)],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )
    done_flag.append(True)  # signal audio finished


def live_countdown(duration, done_flag):
    start = time.time()

    while True:
        elapsed = int(time.time() - start)
        remaining = max(duration - elapsed, 0)

        sys.stdout.write(
            f"\r{colored('Answer in', 'yellow')} ({remaining}) "
        )
        sys.stdout.flush()

        if remaining == 0 and done_flag:
            break

        time.sleep(1)

    print()


# NORMALIZE QUESTIONS
score = 0
for q in questions:
    normalize_audios(q)

# EXAM LOOP
for q in questions:
    print(colored(f"\n🎧 Listening Question {q['id']}", "cyan"))

    # IMAGE
    if q.get("image"):
        img_path = IMAGE_DIR / q["image"]
        if img_path.exists():
            Image.open(img_path).show()

    # CHECK AUDIO FILES
    for a in q["audios"]:
        if not (AUDIO_DIR / a).exists():
            print(colored(f"❌ Missing audio: {a}", "red"))
            print(colored("⏭ Skipping question.", "yellow"))
            continue

    print(colored("🔊 Playing audio...", "yellow"))

    # AUDIO THREAD
    audio_done = []
    audio_thread = threading.Thread(
        target=play_audio_sequence,
        args=(q["audios"], audio_done),
        daemon=True
    )
    audio_thread.start()

    # COUNTDOWN (RUNS WITH AUDIO)
    duration = get_audio_duration(q["audios"])
    live_countdown(duration, audio_done)

    # SHOW QUESTION
    print(colored("\n" + q.get("question", ""), "white"))
    for k, v in q["choices"].items():
        print(f"  ({k}) {v}")

    options = "/".join(q["choices"].keys())

    while True:
        user_answer = input(
            colored(f"\n👉 Your answer ({options}): ", "cyan")
        ).strip().upper()

        if user_answer in q["choices"]:
            break

        print(colored(f"⚠️ Please enter {options} only.", "yellow"))

    # CHECK ANSWER
    if user_answer == q["answer"]:
        print(colored("✅ Correct!", "green"))
        score += 1
    else:
        print(colored(f"❌ Wrong: Correct answer [{q['answer']}]", "red"))

# FINAL SCORE
print(colored(f"\n🎯 Final Score: {score} / {len(questions)}", "magenta"))